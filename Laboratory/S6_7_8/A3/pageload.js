...
function envsClick(event) {
    ...
    $.get("getbyenv.php", {env: event.target.innerHTML}, function(data, status){
        if (status === "success") {
            var html = `<h4 id="center_header"> Environment ${event.target.innerHTML}</h4>`;
            html += '<div id="cards_row" style="display:none;" class="row ui-corner-all">';

            var hotels = $(data).find("hotel");
            for (var i = 0; i < hotels.length; i++) {
                // Draw each card
                var columns = 12 / hotels.length;
                console.log(hotels[i]);
                var [name, city, country, zone, pool, img] = $(hotels[i]).children(); // Destructuring_assignment
                html += `<div class="col-sm-${columns}">
                        <div class="card">
                        <img src="${$(img).attr("src")}" class="card-img-top">
                        <div class="card-body">
                            <h5 class="card-title">${$(name).text()}</h5>
                            <p class="card-text"> ${$(city).text()}, ${$(country).text()}</p>
            ...
            html += '</div>';
            // Update page and show results
            $("#center_body").empty(); // Remover elementos jQuery
            $("#center_body").append(html);
